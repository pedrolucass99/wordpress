...
class Options{
    ...
    public function plugin_action_links_callback ( $links ) {
        ...
        if(!defined('WOPB_PRO_VER')){
            $upgrade_link = array(
                'wopb_pro' => '<a href="'.esc_url(wopb_function()->get_premium_link("", 'plugin_list_productx_go_pro')).'" target="_blank"><span style="color: #e83838; font-weight: bold;">'.esc_html__('Go Pro', 'product-blocks').'</span></a>'
            );
        }
    ...
    public static function menu_page_callback() {
        ...
        if( !function_exists('wopb_pro_function') ){
            global $submenu;
            $submenu['wopb-settings'][] = array( '<span class="wopb-dashboard-upgrade"><span class="dashicons dashicons-update"></span> Go Pro</span>', 'manage_options', esc_url(wopb_function()->get_premium_link("", 'main_menu_go_pro')) );
        }
    }
        ...
        if ($current) {
            ...
            echo '<div class="wopb-settings-tab-wrap">';
                if (date('U') < strtotime('28-04-2023')) {
                    ...
                    if (!$pro_active) {
                        echo '<div class="wopb-setting-hellobar"><span class="dashicons dashicons-bell wopb-ring"></span><strong> Hot Sale: </strong> Up to <b>35% Off</b>. Unlock all premium features by<a href="' . esc_url(wopb_function()->get_premium_link('', 'menu_topbar')) . '" target="_blank">' . esc_html__('Upgrading to Pro', 'product-blocks') . '</a></div>';
                    }
                }
