...
class Functions{
    ...
    public function get_query($attr) {
        ...
        if (!$this->is_specific_archive_product($attr) && $this->is_builder($builder) && !is_product() && !is_cart()) {
            if ($builder) {
                $str = explode('###', $builder);
                if (isset($str[0])) {
                    if ($str[0] == 'taxonomy') {
    ...
    public function pro_popup_html() { ?>
        <div class="wopb-popup-container wopb-unlock-popup-container popup-center">
            <div class="wopb-unlock-popup wopb-unlock-modal">
                ...
                <a href="<?php echo esc_url(wopb_function()->get_premium_link('', 'menu_addons_upgrade_popup')); ?>" class="wopb-popup-planbtn wopb-btn wopb-btn-warning wopb-mt25" target="_blank"><?php esc_html_e('Upgrade Plan', 'product-blocks'); ?></a>
